...
globalDispatch.on('change:country', (code, displayName, migrationData) => {
	...
	renderLineCharts(groupBySubregionByYear(code, migrationData)); //TAKE MIGRATION DATA AND GROUP ALL THE FLOWS BY YEAR AND SUB
	renderComposition(migrationData.filter(d => d.origin_code === code), 2010);
	renderCartogram(migrationData.filter(d => d.origin_code === code));
}); //CALL BACK FUNCTION 
globalDispatch.on('change:year', year=> {
	console.log("globalDispatch" + year);//RE-RENDER THE COMPOSITION AND CARTOGRAM MODULES
...
function renderLineCharts(data){
	...
	const lineChart = LineChart() //THIS RETURN THE INNER FUNCTION, EXPORTING FUNCTION 
		.maxY(maxValue)
		.onChangeYear( //STRING, THE EVENT TYPE
			year => globalDispatch.call("change:year", null, year)); //CALL BACK FUNCTION TO BE EXECUTED UPON THE EVENT 
...
function renderComposition(data){
	...
	.year(2015); //RETURN exportFunction any arbitory year we would like to enter 
